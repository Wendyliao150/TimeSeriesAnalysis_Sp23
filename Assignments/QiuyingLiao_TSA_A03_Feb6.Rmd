...
energy_data_df <- data.frame(energy_data$Month, energy_data$`Total Biomass Energy Production`,energy_data$`Total Renewable Energy Production`, energy_data$`Hydroelectric Power Consumption`)
...
for(i in 1:3){
  par(mfrow = c(1,3))
  #ggplot(energy_data_df, aes(x=energy_data.Month, y=energy_data_df[,(1+i)])) +
            #geom_line(color="blue")
  Acf(ts_energy_data_df[,i], lag.max = 40, main = paste0("Acf of HP",i), ylim=c(-1,1)) #or main= name[i+1]
   Pacf(ts_energy_data_df[,i], lag.max = 40, main = paste0("Pacf of HP",i))#pasting the character HP and index i
}
```
