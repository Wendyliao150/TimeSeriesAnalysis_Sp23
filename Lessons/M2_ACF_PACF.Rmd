...
my_date <- my(my_date)  #function my from package lubridate #my means month-year?, ym means year-month #you can also need as.date, but lubridate is better because as.date need exact digit
head(my_date)
#add that to inflow_data 
inflow_data <- cbind(my_date,raw_inflow_data[,3:5]) #all columns from 3 to 5 
head(inflow_data)
...
# if In-line, in here; if in console, then in the plot function (右下角)
...
ggplot(inflow_data, aes(x=my_date, y=Inflow)) +
            ...
            geom_line(aes(y=HP2,col="HP2")) +   # use line so two plots can be in the same plot
            xlab("Time") +
            labs(color="Reservoir") #rename the legend using "labs"
```
...
```{r}
ts_inflow_data <- ts(inflow_data[,2:(2+nhydro-1)]) #note that we are only transforming columns with inflow data, not the date columns  #(2+nhydro-1) add up equal to 4; can also be nhydro +1, same result
ts_inflow_data
#plot(ts_inflow_data[,1])


#start = the first of your time series
#frequency = when your data repeating itself, frequency of monthly is 12
```
...
```{r}
acf(ts_inflow_data[,1])  # x is our time series #from the plot, we see the seasonal component here # the blue line is significant level, inside the blue line is not significant 
#the first acf is always 1
#acf only takes time series data

#if you want to see lag
acf(ts_inflow_data[,1], lag.max = 50)

#pacf
pacf(ts_inflow_data[,1])

#capital acf and pacf: start at lag 1 instead of lag 0, because lag 0 is 1 
