...
#es; you can change your parameter over time
```
...
#SS_seas <- StructTS(ts_price,
                    #type="BSM",fixed=c(NA,NA,NA,NA)) #put NA to let them help us find the values (NA allows it to try different variance)

#for ACF: our error are correlated, not good, so try differernt value (below)

#the four variation:
#1. level of the series 
#2. trend of the series
#3. seasonal 
#4. observation: Yt
...
#=c(0,0.001,0.3,NA)) keep playing with this model until the model looks good
#how your residual are correlated, we want residual to be uncorrelated when fitting a model
...
#if we allow our variance to have more, we did a better job in fitting models, but it also leads to many problems such as convergence
