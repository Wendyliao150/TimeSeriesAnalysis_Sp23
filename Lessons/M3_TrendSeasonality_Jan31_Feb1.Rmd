...
for(i in 1:nhydro){
  par(mfrow=c(1,2))
  Acf(ts_inflow_data[,i],lag.max = 40,main=name[i+1],ylaim=c(-1,1))
...
dummies <- seasonaldummy(ts_inflow_data[,iHP]) #only have 11, have to leave one out for comparison
# run with intersect, then you will have 11

seas_means_model <- lm(inflow_data[,(iHP+1)]~dummies) #lm function only take data frame  # we used the orignal data (we detrend from original data in this case)
summary(seas_means_model)
# mean for january is the intercept + the value of january*coefficient
# intercept = mean of December
...
#works for short time period series

#fit a constant seasonal model
#eliminate trend, seasonality

#diversify for trend and seasonality
...
#Mann Kendall
#null:dataset is stationary (no trend)
#Ha: dataset have trend

#how do we know? use p-value
