...
iHP=1
decompose_inflow_data=decompose(ts_inflow_data[,iHP],"additive") #specify frequency when you do ts for decompose
...
#if you sum these three, you get the observed one (random +seasonal+trend)

#decompose can be extracted random, seasonal, trend,etc
...
ggplot(inflow_random, aes(x=date, y=inflow)) +
            ...
            geom_abline(slope=0,intercept=-3*sd_inflow,color="red") #outside of red line is outlier

ggplot(inflow_random, aes(y=inflow)) +
            geom_boxplot()  #two dots at the top
```
...
#mean without the outlier removed?
```
