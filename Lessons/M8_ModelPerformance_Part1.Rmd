...
## Fitting Models to the original (seasonal) series (backward looking model)
...
#ACF graph: have strong autoregression component, show that not independent, not good 
```
...
#histogram: more look like normal distribution, much better
...
## Model Performance for forecasting 12 steps ahead (foreward looking)
...
If **x** is not provided the function will return performance measures for training set, i.e., based on historical data it will compare observed and fitted values.
...
#last 12 elements:(nobs-n_for+1):nobs

#Model 1: Arithmetic mean
MEAN_scores <- accuracy(MEAN_seas$mean,observed)  #store the performance metrics #compare with observed
plot(MEAN_seas)  #plot forecasts

#backward have more metrics than foreward one
...
SARIMA_for <- forecast(SARIMA_autofit,h=12) #we are interested in the mean
...
#last column: mean absolute percentage error: SARIMA has the best error. we want value to be small as possible. 
...
#kbl function: to make dataframe
```
