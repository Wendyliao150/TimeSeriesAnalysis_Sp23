...
```{r}
ts_inflow_data1 <- ts(inflow_data[,2:16], start = c(1931,1), end = c(1977,8), frequency =12)
# you can use 2+nhydro-1 for 16 so that you don't have to count column one by one
head(ts_inflow_data1, 20)
```
...
for(i in 1:nhydro){
  par(mfrow = c(1,2))
  Acf(ts_inflow_data1[,i], lag.max = 40, main = paste0("Acf of HP",i), ylim=c(-1,1)) #or main= name[i+1]
  Pacf(ts_inflow_data1[,i], lag.max = 40, main = paste0("Pacf of HP",i))#pasting the character HP and index i
}


#interpretation
#1. non stationarity in the series
#2. my correlation change side as the period change
# analytics is increasing
# 7 is a example of strong correlation,

# look at y-axis to look at the magnitude of correlation
#seasonality
# HP5, correlation is decreasing over time, 
# HP4, they are all positive, no obvious seasonality, it is decling, there is a trend in the series that need to be removed so that we need to see the fact of acf
# HP3, even at 36, still have strong correlation with lag variable 

# correlation is still strong even though at 40 lag 

#hp8: have a weak autocorrelation, getting closer to the blue line



# Pacf is always smaller than Acf
...
#Create vector t
t <- 1:nobs
iHP=4 # you just refer to that column, create a variable to represent the column to model 
