...
cat("Number of differencing needed: ",n_diff) #you need to difference at lag 1
...
residential_price_seas_diff <- diff(ts_electricity_price[,"Residential"],lag=12, differences=1) #difference at seasonal lag, differncing 1
...
Acf(residential_price_seas_diff,lag.max=60,main="Seasonal-Differenced Residential",ylim=c(-1,1)) #at lag 12
Acf(residential_price_trend_diff,lag.max=60,main="Trend-Differenced Residential",ylim=c(-1,1)) #at lag 1
...
#D=1, P=0 (seasonal moving average), Q=1
...
#AIC:-234.11 is this show ARIMA model is better?
#they are not comparable, because we are using different models.
#different approach

#we assume that seasonal component is constant, we remove that to do this model
#how this perform on the non-seasonal componenet only

#for the forecast model, we will put the seasonal component back to do analysis
